        ...
        <div class="card-box table-responsive">
            ...
            <h4 class="header-title m-t-0 m-b-30"><i class="fa fa-tree"></i> <?php echo $sidebar_menu; ?>&nbsp;
                ...
                <ul class="list-status">
                    ...
                    <li>
                        <button title="Add Staff" data-tooltip="true" type="button"
                                class="btn btn-primary waves-effect waves-light" onclick="javascript:add_staff();"><i
                    ...
                    foreach ($user_data as $user) {
                        if ($user->status == '1') {
                            $status = '<div class="active-user">
														<a  onclick="status(' . $user->id . ',' . $user->status . ')" id="' . $user->id . '" title="Click to set inactive" data-tooltip="true" href="javascript:void(0)"> <i class="fa fa-check-circle text-success"></i> </a> 
													 </div>';
                        } elseif ($user->status == '0') {
                            $status = '<div class="deactive-user">
														<a  onclick="status(' . $user->id . ',' . $user->status . ')" id="' . $user->id . '" title="Click to set active" data-tooltip="true" href="javascript:void(0)"> <i class="fa fa-check-circle text-unsuccess"></i> </a> 
													 </div>';
                        }
                        ?>
                        <tr>
                            ...
                            <td class="actions">
                                ...
                                <button title="Edit Staff" data-tooltip="true" type="button"
                                        class="on-default edit-row"
                                        onclick="javascript:edit_staff(<?php echo $user->id; ?>);">
                                    ...
                                    <div class="modal-dialog modal-sm">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                Are you confirm to delete this staff?
                                            </div>
                                            <div class="modal-footer">
                                                ...
                                                <button type="button"
                                                        ...
                                                        onclick="delete_staff('<?php echo $user->id; ?>')">
...
<script type="text/javascript">
    function edit_staff(id) {
        window.location.href = '<?php echo base_url();?>users/edit_staff/' + id;
    }

    function add_staff() {
        window.location.href = '<?php echo base_url();?>users/add_staff';
    }

    function delete_staff(id) {
        window.location.href = '<?php echo base_url();?>users/delete_staff/' + id;
    }
            ...
            success: function (data, textStatus, XMLHttpRequest) {
                if (data.valid) {
                    ...
                    if (data.success_message == 1) {
                        var st = 1;
                        $('#status_' + id).html('<div class="active-user"><a onclick="status(' + id + ',' + st + ')" id="' + id + '" title="Click to set inactive" data-tooltip="true" href="javascript:void(0)"> <i class="fa fa-circle text-success"></i> </a></div>');
                        ...
                        var st = 0;
                        $('#status_' + id).html('<div class="deactive-user"><a onclick="status(' + id + ',' + st + ')" id="' + id + '" title="Click to set active" data-tooltip="true" href="javascript:void(0)"> <i class="fa fa-circle text-unsuccess"></i> </a></div>');
